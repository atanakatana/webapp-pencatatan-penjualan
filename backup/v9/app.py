...
def seed_db_command():
    """Menghapus database lama dan membuat data dummy untuk 12 bulan terakhir."""
    db.drop_all()
    db.create_all()
    print("Database dibersihkan...")

    # 1. Buat data dasar (Owner, Admin, Lapak, Supplier)
    ...
    db.session.add_all([owner, admin1])
    db.session.commit()
    ...
    print("Data dasar (user, lapak, supplier) berhasil dibuat.")

    # 2. Buat data historis untuk 12 bulan terakhir
    print("Membuat data historis untuk grafik...")
    today = datetime.date.today()
    for i in range(365, 0, -1): # Loop mundur selama setahun
        current_date = today - timedelta(days=i)

        # Buat data LaporanHarian (Pendapatan) dengan probabilitas 80% setiap hari
        if random.random() < 0.8:
            pendapatan_harian = random.randint(500000, 2500000)
            biaya_harian = pendapatan_harian * random.uniform(0.6, 0.8)
            report = LaporanHarian(
                lapak_id=lapak1.id,
                tanggal=current_date,
                total_pendapatan=pendapatan_harian,
                total_biaya_supplier=biaya_harian,
                pendapatan_cash=pendapatan_harian / 2,
                pendapatan_qris=pendapatan_harian / 2,
                pendapatan_bca=0,
                total_produk_terjual=random.randint(50, 250),
                status='Terkonfirmasi' # PENTING: harus terkonfirmasi agar masuk grafik
            )
            db.session.add(report)

        # Buat data PembayaranSupplier (Biaya) dengan probabilitas 20% setiap hari
        if random.random() < 0.2:
            pembayaran = PembayaranSupplier(
                supplier_id=supplier1.id,
                tanggal_pembayaran=current_date,
                jumlah_pembayaran=random.randint(200000, 1000000),
    ...
    print("\nDatabase di-seed dengan data lengkap. Silakan login dan cek halaman grafik.")
@app.route('/')
def login_page():
...
@app.route('/api/get_manage_reports') # Nama endpoint diubah
