...
class LaporanHarian(db.Model):
    ...
    bukti_qris_path = db.Column(db.String(255), nullable=True) # Path ke file bukti QRIS jika ada
...
# --- OWNER API ---
@app.route('/api/get_supplier_payment_history/<int:supplier_id>', methods=['GET'])
def get_supplier_payment_history(supplier_id):
    try:
        payments = PembayaranSupplier.query.filter_by(
            supplier_id=supplier_id
        ).order_by(PembayaranSupplier.tanggal_pembayaran.desc()).all()

        history = [{
            "tanggal": p.tanggal_pembayaran.strftime('%d %B %Y'),
        ...
        logging.error(f"Error fetching supplier payment history: {e}")
        return jsonify({"success": False, "message": "Gagal mengambil riwayat pembayaran."}), 500
...
def add_supplier():
    ...
    try:
        ...
        db.session.flush() # Baris ini penting untuk mendapatkan ID supplier baru

        # Logika baru: Alokasi lapak dibaca dari setiap data produk
        for p_data in data.get('products', []):
            if p_data.get('name'):
                lapak_ids = p_data.get('lapak_ids', []) # Ambil lapak_ids dari dalam produk
                lapaks_to_associate = Lapak.query.filter(Lapak.id.in_(lapak_ids)).all()
                ...
                new_product = Product(
                    ...
                    lapaks=lapaks_to_associate # Langsung asosiasikan di sini
                )
...
@app.route('/api/get_supplier_products/<int:supplier_id>', methods=['GET'])
def get_supplier_products(supplier_id):
    """Mengambil semua produk dan alokasi lapaknya untuk satu supplier."""
    try:
        # Menggunakan joinedload untuk efisiensi query ke database
        supplier = Supplier.query.options(
            joinedload(Supplier.products).joinedload(Product.lapaks)
        ).get_or_404(supplier_id)
        ...
        for p in supplier.products:
            product_list.append({
                ...
                "lapak_ids": [lapak.id for lapak in p.lapaks] # Kirim ID lapak yang terhubung
            })
            
        return jsonify({"success": True, "products": product_list})
    except Exception as e:
        logging.error(f"Error getting supplier products: {e}")
        return jsonify({"success": False, "message": "Gagal mengambil data produk."}), 500

@app.route('/api/update_supplier_products/<int:supplier_id>', methods=['PUT'])
def update_supplier_products(supplier_id):
    """Mengupdate, menambah, atau menghapus produk & alokasi lapak untuk satu supplier."""
    data = request.json
    supplier = Supplier.query.get_or_404(supplier_id)
    
    try:
        existing_product_ids = {p.id for p in supplier.products}
        incoming_product_ids = {p['id'] for p in data.get('products', []) if p.get('id')}

        # 1. Hapus produk yang tidak ada lagi di daftar yang dikirim
        ids_to_delete = existing_product_ids - incoming_product_ids
        if ids_to_delete:
            Product.query.filter(Product.id.in_(ids_to_delete)).delete(synchronize_session=False)

        # 2. Proses produk yang masuk (update atau tambah baru)
        for p_data in data.get('products', []):
            lapak_ids = p_data.get('lapak_ids', [])
            lapaks_to_associate = Lapak.query.filter(Lapak.id.in_(lapak_ids)).all()

            if p_data.get('id'): # Jika ada ID, update produk yang sudah ada
                product = Product.query.get(p_data['id'])
                if product:
                    product.nama_produk = p_data['name']
                    product.harga_beli = float(p_data.get('harga_beli', HARGA_BELI_DEFAULT))
                    product.harga_jual = float(p_data.get('harga_jual', HARGA_JUAL_DEFAULT))
                    product.lapaks = lapaks_to_associate # Update alokasi lapak
            else: # Jika tidak ada ID, ini produk baru
                if p_data.get('name'): # Pastikan nama produk tidak kosong
                    new_product = Product(
                        nama_produk=p_data['name'],
                        supplier_id=supplier.id,
                        harga_beli=float(p_data.get('harga_beli', HARGA_BELI_DEFAULT)),
                        harga_jual=float(p_data.get('harga_jual', HARGA_JUAL_DEFAULT)),
                        lapaks=lapaks_to_associate
                    )
                    db.session.add(new_product)
        
        db.session.commit()
        return jsonify({"success": True, "message": "Produk dan alokasi supplier berhasil diperbarui."})
    except Exception as e:
        db.session.rollback()
        logging.error(f"Error updating supplier products: {e}")
        return jsonify({"success": False, "message": "Terjadi kesalahan saat memperbarui data."}), 500

@app.route('/api/get_supplier_payment_invoice/<int:supplier_id>', methods=['GET'])
def get_supplier_payment_invoice(supplier_id):
    try:
        supplier = Supplier.query.get_or_404(supplier_id)
        payments = PembayaranSupplier.query.filter_by(
            supplier_id=supplier_id
        ...
        total_dibayar = sum(p['jumlah'] for p in payment_list)
        ...
        invoice_data = {
            "supplier_info": {
                ...
                "no_register": supplier.nomor_register
            },
        ...
        logging.error(f"Error fetching supplier invoice: {e}")
...
def get_pembayaran_data():
    try:
        # 1. Buat subquery untuk menghitung total pembayaran per supplier
        total_paid_subq = db.session.query(
            PembayaranSupplier.supplier_id,
            func.sum(PembayaranSupplier.jumlah_pembayaran).label('total_dibayar')
        ).group_by(PembayaranSupplier.supplier_id).subquery()

        # 2. Query utama yang menggabungkan data supplier, balance, dan total pembayaran
        suppliers_data = db.session.query(
            Supplier,
            SupplierBalance.balance,
            total_paid_subq.c.total_dibayar
        ).outerjoin(SupplierBalance, Supplier.id == SupplierBalance.supplier_id)\
         .outerjoin(total_paid_subq, Supplier.id == total_paid_subq.c.supplier_id)\
         .order_by(Supplier.nama_supplier).all()
        
        # 3. Format hasilnya ke dalam list dictionary
        result = []
        for supplier, balance, total_dibayar in suppliers_data:
            result.append({
                "supplier_id": supplier.id,
                "nama_supplier": supplier.nama_supplier,
                "total_tagihan": balance if balance is not None else 0.0,
                "total_dibayar": total_dibayar if total_dibayar is not None else 0.0, # Tambahkan data baru
        ...
        logging.error(f"Error getting pembayaran data: {e}")
        return jsonify({"success": False, "message": str(e)}), 500
...
@app.route('/api/get_owner_pembayaran_harian', methods=['GET'])
def get_owner_pembayaran_harian():
    try:
        date_str = request.args.get('date')
        target_date = datetime.datetime.strptime(date_str, '%Y-%m-%d').date()

        payments = PembayaranSupplier.query.options(
            joinedload(PembayaranSupplier.supplier)
        ).filter(
            PembayaranSupplier.tanggal_pembayaran == target_date
        ).order_by(PembayaranSupplier.id.desc()).all()
        ...
        total_harian = sum(p.jumlah_pembayaran for p in payments)
        ...
        logging.error(f"Error fetching owner payment report: {e}")
        return jsonify({"success": False, "message": str(e)}), 500
...
def get_data_buat_catatan(lapak_id):
    ...
    products = Product.query.join(product_lapak_association).join(Lapak).filter(
        Lapak.id == lapak_id,
        Product.is_manual == False # Hanya ambil produk dari supplier, bukan produk manual lapak lain
    ).options(joinedload(Product.supplier)).order_by(Product.supplier_id).all()
    # -----------------------------

    suppliers_data = {}
    for p in products:
        supplier = p.supplier
        # Pastikan produk memiliki supplier, jika tidak, abaikan
        if not supplier:
            continue

        supplier_id = supplier.id
        supplier_name = supplier.nama_supplier
        supplier_reg = supplier.nomor_register
...
def submit_catatan_harian():
    # 🔄 Ambil data JSON yang dikirim sebagai string dari form field 'data'
    try:
        data = json.loads(request.form.get('data'))
    except (json.JSONDecodeError, TypeError):
        return jsonify({"success": False, "message": "Format data laporan tidak valid."}), 400
    ...
    # ➕ Logika untuk menyimpan file upload
    bukti_file_path = None
    if 'bukti_qris' in request.files:
        file = request.files['bukti_qris']
        if file.filename != '':
            # Membuat nama file yang aman dan unik
            filename = secure_filename(f"{lapak_id}_{today.isoformat()}_{file.filename}")
            file_path = os.path.join(app.config['UPLOAD_FOLDER'], filename)
            file.save(file_path)
            bukti_file_path = filename # Simpan nama filenya saja di DB

    try:
        total_pendapatan_auto, total_biaya_auto, total_terjual_auto = 0.0, 0.0, 0
        
        # 🔄 Saat membuat new_report, tambahkan path filenya
        new_report = LaporanHarian(
            ...
            bukti_qris_path=bukti_file_path # ➕ TAMBAHKAN
        )
...
@app.route('/api/get_supplier_penjualan_harian/<int:supplier_id>')
def get_supplier_penjualan_harian(supplier_id):
    try:
        date_str = request.args.get('date')
        target_date = datetime.datetime.strptime(date_str, '%Y-%m-%d').date()

        sales_by_lapak = db.session.query(
            Lapak.lokasi, Product.nama_produk,
        ...
        total_harian = sum(s.total_harga_beli for s in sales_by_lapak)
        laporan_per_lapak = {}
        for sale in sales_by_lapak:
            if sale.lokasi not in laporan_per_lapak:
                laporan_per_lapak[sale.lokasi] = {"total_biaya": 0, "rincian_biaya": []}
            
            laporan_per_lapak[sale.lokasi]["total_biaya"] += sale.total_harga_beli
            laporan_per_lapak[sale.lokasi]["rincian_biaya"].append({
                "produk": sale.nama_produk,
                "jumlah": sale.jumlah_terjual,
                "biaya": sale.total_harga_beli
            })
        
        # Mengubah format dictionary menjadi list
        laporan_list = [{"lokasi": k, **v} for k, v in laporan_per_lapak.items()]

        return jsonify({
            "total_harian": total_harian,
            "laporan_per_lapak": laporan_list
        })
    except Exception as e:
        logging.error(f"Error fetching supplier sales report: {e}")
        return jsonify({"success": False, "message": str(e)}), 500

@app.route('/api/get_supplier_pembayaran_harian/<int:supplier_id>')
def get_supplier_pembayaran_harian(supplier_id):
    try:
        date_str = request.args.get('date')
        target_date = datetime.datetime.strptime(date_str, '%Y-%m-%d').date()
        ...
        payments = PembayaranSupplier.query.filter_by(
            supplier_id=supplier_id,
            tanggal_pembayaran=target_date
        ).order_by(PembayaranSupplier.id.desc()).all()

        payment_list = [{"jumlah": p.jumlah_pembayaran, "metode": p.metode_pembayaran} for p in payments]
        total_harian = sum(p.jumlah_pembayaran for p in payments)
        ...
        logging.error(f"Error fetching supplier payment report: {e}")
        return jsonify({"success": False, "message": str(e)}), 500
