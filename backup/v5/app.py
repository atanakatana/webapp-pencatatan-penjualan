...
class LaporanHarian(db.Model):
    ...
    pendapatan_dana = db.Column(db.Float, nullable=False, default=0) #
    ...
    manual_pendapatan_dana = db.Column(db.Float, nullable=True)
...
def submit_catatan_harian():
    ...
    try:
        ...
        new_report = LaporanHarian(
            ...
            pendapatan_dana=float(data['rekap_pembayaran'].get('dana') or 0), # ➕ TAMBAHKAN INI
            ...
            manual_pendapatan_dana=float(data['rekap_pembayaran'].get('dana') or 0), # ➕ TAMBAHKAN INI
            manual_total_pendapatan=float(data['rekap_pembayaran'].get('total') or 0),
            bukti_qris_path=bukti_file_path
        )
...
def get_data_supplier(supplier_id):
    try:
        ...
        penjualan_bulan_ini = db.session.query(
            ...
            PembayaranSupplier.supplier_id == supplier_id,
            PembayaranSupplier.tanggal_pembayaran >= start_of_month
        ).scalar() or 0
...
@app.route('/api/get_supplier_tagihan_detail/<int:supplier_id>')
def get_supplier_tagihan_detail(supplier_id):
    """Mengembalikan rincian penjualan yang membentuk tagihan."""
    try:
        sales = db.session.query(
            LaporanHarian.tanggal, Lapak.lokasi, Product.nama_produk,
            LaporanHarianProduk.jumlah_terjual, LaporanHarianProduk.total_harga_beli
        ...
        sales_list = [{"tanggal": s.tanggal.strftime('%Y-%m-%d'), "lokasi": s.lokasi, "nama_produk": s.nama_produk, "terjual": s.jumlah_terjual, "tagihan": s.total_harga_beli} for s in sales]
        return jsonify({"success": True, "data": sales_list})
    except Exception as e:
        return jsonify({"success": False, "message": str(e)}), 500

@app.route('/api/get_supplier_pembayaran_detail/<int:supplier_id>')
def get_supplier_pembayaran_detail(supplier_id):
    """Mengembalikan rincian pembayaran yang diterima."""
    try:
        # Menerima argumen bulan dari request, contoh: ?month=2025-09
        month_str = request.args.get('month')
        query = PembayaranSupplier.query.filter_by(supplier_id=supplier_id)
        ...
        if month_str:
            try:
                ...
                query = query.filter(PembayaranSupplier.tanggal_pembayaran.between(start_date, end_date))
            except ValueError:
                # Abaikan jika format bulan salah
                pass
        
        payments = query.order_by(PembayaranSupplier.tanggal_pembayaran.desc()).all()
        payment_list = [{"tanggal": p.tanggal_pembayaran.strftime('%Y-%m-%d'), "jumlah": p.jumlah_pembayaran, "metode": p.metode} for p in payments]
        return jsonify({"success": True, "data": payment_list})
    except Exception as e:
        return jsonify({"success": False, "message": str(e)}), 500
