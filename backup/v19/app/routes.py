...
def handle_login():
    ...
    if admin and admin.password == password:
        ...
        session['user_info'] = {"nama_lengkap": admin.nama_lengkap, "id": admin.id}
        # Tambahan: Ambil lapak_id untuk admin lapak
        if role == 'lapak':
            lapak_info = Lapak.query.filter(or_(Lapak.user_id == admin.id, Lapak.anggota.any(id=admin.id))).first()
            if lapak_info:
                session['user_info']['lapak_id'] = lapak_info.id
    ...
    if supplier and supplier.password == password:
        session['user_role'] = 'supplier'
        session['user_info'] = {"nama_supplier": supplier.nama_supplier,"supplier_id": supplier.id}
...
def get_session_info():
    """Mengembalikan informasi sesi pengguna yang sedang login."""
    if 'user_role' in session:
        return jsonify({
            "success": True,
            "is_logged_in": True,
