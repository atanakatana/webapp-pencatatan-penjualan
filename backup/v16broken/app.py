...
# Tabel penghubung antara Supplier dan Lapak
supplier_lapak_association = db.Table('supplier_lapak',
...
class Supplier(db.Model):
    ...
    lapaks = db.relationship('Lapak', secondary=supplier_lapak_association, lazy='subquery',
...
class LaporanHarian(db.Model):
    ...
    total_keuntungan = db.Column(db.Float, nullable=False, default=0)
...
class LaporanHarianProduk(db.Model):
    ...
    keuntungan = db.Column(db.Float, nullable=False, default=0)
...
class Notification(db.Model):
    ...
    is_read_owner = db.Column(db.Boolean, default=False)
    is_read_supplier = db.Column(db.Boolean, default=False)
    is_action_taken = db.Column(db.Boolean, default=False, nullable=False)
...
def seed_db_command():
    ...
    db.session.flush()
    db.session.add_all([
        Product(nama_produk="Roti Tawar Gandum", supplier_id=supplier_roti.id, harga_beli=8000, harga_jual=10000),
        Product(nama_produk="Roti Sobek Coklat", supplier_id=supplier_roti.id, harga_beli=8000, harga_jual=10000),
        Product(nama_produk="Donat Gula", supplier_id=supplier_roti.id, harga_beli=8000, harga_jual=10000)
    ])
    ...
    db.session.flush()
    db.session.add_all([
        Product(nama_produk="Es Teh Manis", supplier_id=supplier_minuman.id, harga_beli=8000, harga_jual=10000),
        Product(nama_produk="Jus Jambu", supplier_id=supplier_minuman.id, harga_beli=8000, harga_jual=10000),
        Product(nama_produk="Kopi Susu Gula Aren", supplier_id=supplier_minuman.id, harga_beli=8000, harga_jual=10000)
    ])
    ...
    db.session.add_all([
        ...
        Product(nama_produk="Tahu Crispy", supplier_id=supplier_snack.id, harga_beli=8000, harga_jual=10000)
    ])
    
    db.session.commit()
    print("=> 3 Supplier dengan total 8 produk berhasil dibuat.")

        ## 4. Hubungkan Supplier ke Lapak (LOGIKA BARU YANG LEBIH SPESIFIK)
    # ===================================================================
    # Kita buat data demo lebih realistis
    # Lapak Kopo menjual produk dari Roti dan Minuman
    lapak_kopo.suppliers.append(supplier_roti)
    lapak_kopo.suppliers.append(supplier_minuman)
        
    # Lapak Buah Batu menjual produk dari Roti dan Snack
    lapak_buah_batu.suppliers.append(supplier_roti)
    lapak_buah_batu.suppliers.append(supplier_snack)

    db.session.commit()
    print("=> Supplier demo berhasil dihubungkan ke lapak demo secara spesifik.")
    ...
    for i in range(90, 0, -1):
        ...
        for lapak in all_lapaks:
            if random.random() < 0.85:
                ...
                total_keuntungan_harian = 0 # Variabel baru
                ...
                for product in products_for_the_day:
                    ...
                    keuntungan_produk = total_harga_jual - total_harga_beli # PERBAIKAN: Hitung keuntungan
                    ...
                    rincian = LaporanHarianProduk(laporan_id=report.id, product_id=product.id,
                                                  ...
                                                  keuntungan=keuntungan_produk) # PERBAIKAN: Simpan keuntungan
                    ...
                    total_keuntungan_harian += keuntungan_produk # PERBAIKAN: Akumulasi keuntungan
                ...
                report.total_keuntungan = total_keuntungan_harian # PERBAIKAN: Simpan total keuntungan
...
def add_supplier():
    ...
    try:
        ...
        db.session.flush()  # Dapatkan ID supplier baru

        # PERBAIKAN: Kembalikan logika untuk menambah produk
...
# GANTI update_supplier DENGAN INI
# ...DENGAN VERSI BARU YANG LENGKAP INI
...
def update_supplier(supplier_id):
    ...
    try:
        # 1. Update data dasar supplier (tanpa produk)
        ...
        # Logika untuk memproses produk telah dihapus dari endpoint Owner
...
@app.route('/api/get_notifications_owner', methods=['GET'])
def get_notifications_owner():
    try:
        notifications = Notification.query.options(
            joinedload(Notification.lapak),
        ...
        notif_list = [{
            "id": n.id,
            "message": n.message,
            "lapak": n.lapak.lokasi,
            "timestamp": n.timestamp.strftime('%d %b, %H:%M'),
            "is_read": n.is_read_owner,
            "is_action_taken": n.is_action_taken
        } for n in notifications]
        
        unread_count = Notification.query.filter_by(is_read_owner=False).count()
        
        return jsonify({"success": True, "notifications": notif_list, "unread_count": unread_count})
    except Exception as e:
        return jsonify({"success": False, "message": str(e)}), 500
    
@app.route('/api/mark_notification_action/<int:notification_id>', methods=['POST'])
def mark_notification_action(notification_id):
    try:
        ...
        notification.is_action_taken = True
        db.session.commit()
        
        return jsonify({"success": True, "message": "Tindakan telah dikonfirmasi."})
    except Exception as e:
        db.session.rollback()
        logging.error(f"Error marking notification action: {e}")
        return jsonify({"success": False, "message": "Gagal mengkonfirmasi tindakan."}), 500
# --- LAPAK API ---
# --- REVISI: Kirim info pembayaran ke frontend ---
# --- TAMBAHKAN 4 FUNGSI BARU INI ---

@app.route('/api/lapak/suppliers_products/<int:lapak_id>', methods=['GET'])
def get_lapak_suppliers_products(lapak_id):
    """Mengambil semua supplier & produk yang terhubung dengan lapak ini."""
    try:
        lapak = Lapak.query.get_or_404(lapak_id)
        suppliers = lapak.suppliers # Menggunakan backref dari relasi baru
...
@app.route('/api/lapak/add_supplier', methods=['POST'])
def add_lapak_supplier():
    """Admin lapak menambah supplier baru. Cek duplikat global."""
    data = request.json
    lapak_id = data.get('lapak_id')
    nama_supplier = data.get('nama_supplier', '').strip()

    if not all([lapak_id, nama_supplier]):
    ...
    try:
        ...
        # Cek apakah supplier dengan nama ini sudah ada di database (global)
        supplier = Supplier.query.filter(func.lower(Supplier.nama_supplier) == func.lower(nama_supplier)).first()
        
        if not supplier:
            # Jika supplier belum ada sama sekali, buat baru
            supplier = Supplier(
                nama_supplier=nama_supplier,
                kontak=data.get('kontak'),
                username=f"sup_{random.randint(1000, 9999)}", # Username random
                password="password" # Default password, bisa diubah owner
            )
            supplier.balance = SupplierBalance()
            db.session.add(supplier)
            db.session.flush() # Butuh ID untuk relasi produk

        # Hubungkan supplier (baik yang baru dibuat maupun yang sudah ada) ke lapak ini
        if supplier not in lapak.suppliers:
            lapak.suppliers.append(supplier)

        # Proses produk
        for p_data in data.get('products', []):
            nama_produk = p_data.get('name', '').strip()
            if not nama_produk: continue
            
            # Cek apakah produk dengan nama ini sudah ada untuk supplier ini
            product_exists = Product.query.filter(
                Product.supplier_id == supplier.id,
                func.lower(Product.nama_produk) == func.lower(nama_produk)
            ).first()
        ...
        db.session.commit()
        return jsonify({"success": True, "message": f"Supplier '{nama_supplier}' berhasil disimpan dan dihubungkan ke lapak Anda."})

    except Exception as e:
        db.session.rollback()
        return jsonify({"success": False, "message": str(e)}), 500

@app.route('/api/lapak/add_stok', methods=['POST'])
def add_stok():
    data = request.json
    lapak_id = data.get('lapak_id')
    ...
    if not all([lapak_id, product_id, jumlah]):
        return jsonify({"success": False, "message": "Data tidak lengkap."}), 400
    ...
    try:
        ...
        # Buat notifikasi untuk supplier
        if product.supplier_id:
            notif_message = f"Stok '{product.nama_produk}' sejumlah {jumlah} pcs telah diterima."
            notification = Notification(
                lapak_id=lapak_id,
                product_id=product_id,
                supplier_id=product.supplier_id,
                message=notif_message,
                is_action_taken=True # Dianggap sudah ditindaklanjuti
            )
            db.session.add(notification)

        db.session.commit()
        return jsonify({"success": True, "message": "Stok masuk berhasil dicatat."})
    except Exception as e:
        db.session.rollback()
        return jsonify({"success": False, "message": str(e)}), 500

@app.route('/api/lapak/stok_masuk_history/<int:lapak_id>', methods=['GET'])
def get_stok_masuk_history(lapak_id):
    try:
        history = StokMasuk.query.options(
            joinedload(StokMasuk.product).joinedload(Product.supplier)
        ).filter_by(lapak_id=lapak_id).order_by(StokMasuk.tanggal.desc(), StokMasuk.id.desc()).limit(50).all()

        history_list = [{
            "tanggal": h.tanggal.strftime('%Y-%m-%d'),
            "nama_produk": h.product.nama_produk,
            "supplier": h.supplier.nama_supplier if h.supplier else "N/A",
            "jumlah": h.jumlah
        } for h in history]
...
@app.route('/api/lapak/update_supplier/<int:supplier_id>', methods=['PUT'])
def update_lapak_supplier(supplier_id):
    """Admin lapak mengedit supplier yang terhubung dengannya, termasuk produk."""
    data = request.json
    lapak_id = data.get('lapak_id')
    
    if not lapak_id:
        return jsonify({"success": False, "message": "ID Lapak dibutuhkan."}), 400

    lapak = Lapak.query.get(lapak_id)
    supplier = Supplier.query.get_or_404(supplier_id)

    # Keamanan: Pastikan supplier ini terhubung ke lapak yang mengedit
    if supplier not in lapak.suppliers:
        return jsonify({"success": False, "message": "Akses ditolak."}), 403
    
    try:
        # 1. Update info dasar supplier (hanya yang relevan bagi admin lapak)
        supplier.nama_supplier = data['nama_supplier']
        supplier.kontak = data.get('kontak')

        # 2. Logika manajemen produk (dipindahkan dari endpoint Owner)
        submitted_product_ids = {p.get('id') for p in data.get('products', []) if p.get('id')}

        # Hapus produk yang tidak lagi dikirim
        ...
        for p_data in data.get('products', []):
            ...
            if product_id: # Produk lama -> UPDATE
                ...
                if product_to_update and product_to_update.supplier_id == supplier_id:
                    product_to_update.nama_produk = nama_produk
...
def get_data_buat_catatan(lapak_id):
    ...
    try:
        ...
        # 1. Ambil stok akhir dari laporan kemarin
        stok_akhir_kemarin = {
            item.product_id: item.stok_akhir
            for item in LaporanHarianProduk.query
                .join(LaporanHarian)
                .filter(LaporanHarian.lapak_id == lapak_id, LaporanHarian.tanggal == yesterday)
        }

        # 2. Ambil total stok masuk hari ini
        stok_masuk_hari_ini = {
            r[0]: r[1] for r in db.session.query(StokMasuk.product_id, func.sum(StokMasuk.jumlah))
                .filter(StokMasuk.lapak_id == lapak_id, StokMasuk.tanggal == today)
                .group_by(StokMasuk.product_id)
        }

        # 3. Ambil supplier & produk yang terhubung ke lapak ini
        suppliers_data = []
        for s in lapak.suppliers:
            products_list = []
            for p in s.products:
                # 4. Hitung stok awal otomatis
                sisa_kemarin = stok_akhir_kemarin.get(p.id, 0)
                masuk_hari_ini = stok_masuk_hari_ini.get(p.id, 0)
                stok_awal_otomatis = sisa_kemarin + masuk_hari_ini
                ...
                products_list.append({
                    ...
                    "harga_jual": p.harga_jual,
                    "harga_beli": p.harga_beli,
                    "stok_awal_otomatis": stok_awal_otomatis # Kirim data ini ke frontend
                })
...
def submit_catatan_harian():
    ...
    try:
        # Kalkulasi ulang stok awal di backend untuk validasi
        stok_akhir_kemarin = {
            item.product_id: item.stok_akhir
            for item in LaporanHarianProduk.query
                .join(LaporanHarian)
        ...
        total_pendapatan_auto, total_biaya_auto, total_terjual_auto, total_keuntungan_auto = 0.0, 0.0, 0, 0.0
        
        new_report = LaporanHarian(lapak_id=lapak_id, tanggal=today, #... (Sama seperti sebelumnya)
        # ... (Sisa inisialisasi LaporanHarian sama)
        )
        ...
        for prod_data in data.get('products', []):
            ...
            # --- LOGIKA INTI BARU ---
            sisa_kemarin = stok_akhir_kemarin.get(product.id, 0)
            masuk_hari_ini = stok_masuk_hari_ini.get(product.id, 0)
            stok_awal = sisa_kemarin + masuk_hari_ini

            # Jika stok akhir lebih besar dari stok awal yang dihitung, lewati (data tidak valid)
            if stok_akhir > stok_awal: continue

            jumlah_terjual = stok_awal - stok_akhir
            if jumlah_terjual <= 0: continue # Hanya catat yang terjual

            total_harga_jual = jumlah_terjual * product.harga_jual
            total_harga_beli = jumlah_terjual * product.harga_beli
            keuntungan_produk = total_harga_jual - total_harga_beli

            rincian = LaporanHarianProduk(
                laporan_id=new_report.id, product_id=product.id,
                stok_awal=stok_awal, # Simpan stok awal yang dihitung
                stok_akhir=stok_akhir, # Simpan stok akhir dari input
                jumlah_terjual=jumlah_terjual, 
                total_harga_jual=total_harga_jual, total_harga_beli=total_harga_beli,
                keuntungan=keuntungan_produk
            )
            db.session.add(rincian)
            ...
            total_keuntungan_auto += keuntungan_produk
        ...
        new_report.total_keuntungan = total_keuntungan_auto
...
@app.route('/api/submit_stock_alert', methods=['POST'])
def submit_stock_alert():
    data = request.json
    try:
        product_id = data.get('product_id')
        lapak_id = data.get('lapak_id')
        ...
        message = f"Stok '{product.nama_produk}' menipis/habis. Sisa: {remaining_stock} pcs."
        ...
        return jsonify({"success": True, "message": "Laporan stok berhasil dikirim."})
    except Exception as e:
        db.session.rollback()
        logging.error(f"Error submitting stock alert: {e}")
        return jsonify({"success": False, "message": "Gagal mengirim laporan stok."}), 500
...
@app.route('/api/get_notifications_supplier/<int:supplier_id>', methods=['GET'])
def get_notifications_supplier(supplier_id):
    try:
        notifications = Notification.query.options(
            joinedload(Notification.lapak),
            joinedload(Notification.product)
        ).filter_by(supplier_id=supplier_id).order_by(Notification.timestamp.desc()).limit(20).all()
        ...
        notif_list = [{
            ...
            "lapak": n.lapak.lokasi,
            "timestamp": n.timestamp.strftime('%d %b, %H:%M'),
            "is_read": n.is_read_supplier,
            "is_action_taken": n.is_action_taken
        } for n in notifications]
        
        unread_count = Notification.query.filter_by(supplier_id=supplier_id, is_read_supplier=False).count()
    ...
    try:
        ...
        for item in report.rincian_produk:
            ...
            rincian_per_supplier[supplier_name].append({
                ...
                "keuntungan": item.keuntungan # Kirim data keuntungan per produk
            })
        ...
        data = {
            ...
            "rekap_otomatis": {
                ...
                "total_keuntungan": report.total_keuntungan # Kirim data total keuntungan
            },
            "rekap_manual": {
